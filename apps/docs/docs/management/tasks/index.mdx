...
## Media journey correlation

The fork adds a `mediaTraceCorrelation` task that samples connected media integrations and builds title traces for [Manage → Media Journey](/docs/management/media-journey) and the [Media journey](/docs/widgets/media-journey) widget. Run it from the Media Journey page or **Manage → Tools → Tasks** if the console is empty after you connect Sonarr, Radarr, a request service, or a download client.
