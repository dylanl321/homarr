...
The fork adds a `mediaTraceCorrelation` task that samples connected media integrations and builds title traces for the [Media journey](/docs/widgets/media-journey) widget. Run it from **Manage → Tools → Tasks** if the widget is empty after you connect Sonarr, Radarr, a request service, or a download client.
